...
int Database::insertFace(std::string userId, std::string imgPath, std::string src_id, float embedding[]) {
    std::string sql;
    sqlite3_stmt *stmt;
    int rc;

    // Prepare
    sql = "INSERT INTO FACE (USR_ID, IMG_PATH, SRC_ID, EMBEDDING) VALUES (?, ?, ?, ?);";
    rc = sqlite3_prepare_v2(m_db, sql.c_str(), -1, &stmt, NULL);
    if (rc != SQLITE_OK) {
        std::cout << "SQL error: " << sqlite3_errmsg(m_db);
        return -1;
    ...
    rc = sqlite3_bind_text(stmt, 3, src_id.c_str(), src_id.length(), SQLITE_STATIC);
    if (rc != SQLITE_OK) {
        std::cout << "SQL bind error: " << sqlite3_errmsg(m_db);
        return -2;
    }
    rc = sqlite3_bind_blob(stmt, 4, embedding, sizeof(float) * m_embedding_dim, SQLITE_STATIC);
    if (rc != SQLITE_OK) {
        std::cout << "SQL bind error: " << sqlite3_errmsg(m_db);
        return -2;
    }
